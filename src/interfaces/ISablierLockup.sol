    ...
    ISablierLockupState // 0 inherited components
{
    ...
    /// @param batchParams Struct encapsulating the function parameters, which are documented in {DataTypes}.
    /// @return streamIds The IDs of the newly created streams.
    function createWithDurationsLD(
        IERC20 token,
        LockupDynamic.CreateWithDurations[] calldata batchParams
    )
        external
        payable
    ...
    function createWithDurationsLL(
        IERC20 token,
        LockupLinear.CreateWithDurations[] calldata batchParams
    )
        external
        payable
        returns (uint256[] memory streamIds);
    ...
    function createWithDurationsLT(
        IERC20 token,
        LockupTranched.CreateWithDurations[] calldata batchParams
    )
        external
        payable
        returns (uint256[] memory streamIds);
    ...
    /// Requirements:
    /// - Must not be delegate called.
    /// - Each stream's `depositAmount` must be greater than zero.
    /// - `timestamps.start` must be greater than zero and less than the first segment's timestamp for each stream.
    /// - Each stream's `segments` must have at least one segment.
    /// - The segment timestamps must be arranged in ascending order for each stream.
    /// - Each stream's `timestamps.end` must be equal to the last segment's timestamp.
    /// - The sum of the segment amounts must equal the deposit amount for each stream.
    /// - Each stream's `recipient` must not be the zero address.
    /// - Each stream's `sender` must not be the zero address.
    /// - `msg.sender` must have allowed this contract to spend at least the sum of `depositAmount` tokens for each
    /// stream.
    /// - Each stream's `token` must not be the native token.
    /// - Each stream's `shape.length` must not be greater than 32 characters.
    ///
    /// @param token The contract address of the ERC-20 token to be distributed.
    /// @param batchParams Struct encapsulating the function parameters, which are documented in {DataTypes}.
    /// @return streamIds The IDs of the newly created streams.
    function createWithTimestampsLD(
        IERC20 token,
        LockupDynamic.CreateWithTimestamps[] calldata batchParams
    )
        external
        payable
        returns (uint256[] memory streamIds);
    ...
    /// - Must not be delegate called.
    /// - Each stream's `depositAmount` must be greater than zero.
    /// - `timestamps.start` must be greater than zero and less than `timestamps.end` for each stream.
    /// - If set, `cliffTime` must be greater than `timestamps.start` and less than `timestamps.end` for each stream.
    /// - Each stream's `recipient` must not be the zero address.
    /// - Each stream's `sender` must not be the zero address.
    /// - The sum of `unlockAmounts.start` and `unlockAmounts.cliff` must be less than or equal to the deposit amount
    /// for
    /// each stream.
    /// - For each stream, if `timestamps.cliff` is not set, the `unlockAmounts.cliff` must be zero.
    /// - `msg.sender` must have allowed this contract to spend at least the sum of `depositAmount` tokens for each
    /// stream.
    /// - Each stream's `token` must not be the native token.
    /// - Each stream's `shape.length` must not be greater than 32 characters.
    ...
    function createWithTimestampsLL(
        IERC20 token,
        LockupLinear.CreateWithTimestamps[] calldata batchParams
    )
        external
        payable
        returns (uint256[] memory streamIds);
    ...
    /// - Must not be delegate called.
    /// - Each stream's `depositAmount` must be greater than zero.
    /// - `timestamps.start` must be greater than zero and less than the first tranche's timestamp for each stream.
    /// - Each stream's `tranches` must have at least one tranche.
    /// - The tranche timestamps must be arranged in ascending order for each stream.
    /// - `timestamps.end` must be equal to the last tranche's timestamp for each stream.
    /// - The sum of the tranche amounts must equal the deposit amount for each stream.
    /// - Each stream's `recipient` must not be the zero address.
    /// - Each stream's `sender` must not be the zero address.
    /// - `msg.sender` must have allowed this contract to spend at least the sum of `depositAmount` tokens for each
    /// stream.
    ...
    function createWithTimestampsLT(
        IERC20 token,
        LockupTranched.CreateWithTimestamps[] calldata batchParams
    )
        external
        payable
        returns (uint256[] memory streamIds);
